	...
	for i, arg := range args {
		...
		if mode == "init-failure" {
			...
			code := func() int {
				...
				return Run("dev", []string{"--config", path}, input, os.Stdout, os.Stderr)
			}()
		...
		if mode == "read-failure" {
			...
			os.Exit(Run("dev", []string{"--config", path}, lifecycleReadFailure{os.Stdin}, output, lifecycleDiagnostics{os.Stderr, before, output}))
		}
