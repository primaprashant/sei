...
func TestReleaseArchives(t *testing.T) {
	...
	source, err := os.ReadFile(repoPath(t, "scripts/install.sh"))
			...
			t.Run(goos+"/"+arch, func(t *testing.T) {
				...
				for {
					...
					case "README.md", "LICENSE", "THIRD_PARTY_NOTICES":
						want, err := os.ReadFile(repoPath(t, h.Name))
				...
				notices, err := os.ReadFile(repoPath(t, "THIRD_PARTY_NOTICES"))
				if err != nil {
					t.Fatal(err)
				}
				moduleSums, err := os.ReadFile(repoPath(t, "go.sum"))
				if err != nil {
					t.Fatal(err)
				}
