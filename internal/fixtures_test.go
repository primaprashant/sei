...
// repoPath resolves repository resources from the internal package's test directory.
// Absolute paths (including externally supplied release archives) are preserved.
func repoPath(t *testing.T, path string) string {
	t.Helper()
	if filepath.IsAbs(path) {
		return path
	}
	absolute, err := filepath.Abs(filepath.Join("..", path))
	if err != nil {
		t.Fatal(err)
	}
	return absolute
