...
func TestBuildVersion(t *testing.T) {
	...
	code := Run("0.2.0-test", []string{"--version"}, strings.NewReader(""), &stdout, &stderr)
	if code != 0 || stdout.String() != "sei 0.2.0-test\n" || stderr.Len() != 0 {
		t.Fatalf("version: status=%d stdout=%q stderr=%q", code, &stdout, &stderr)
	}
}
...
func TestCLI(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			var stdout, stderr bytes.Buffer
			if got := Run("dev", tt.args, strings.NewReader(""), &stdout, &stderr); got != tt.code {
	...
	for _, kind := range []string{"valid", "missing", "missing parent", "malformed", "invalid paths", "directory", "file ancestor", "unreadable", "unreadable ancestor", "dangling", "dangling ancestor", "valid symlink", "symlink loop"} {
		t.Run(kind, func(t *testing.T) {
			...
			for _, explicit := range []bool{false, true} {
				...
				var stdout, stderr bytes.Buffer
				code := Run("dev", args, strings.NewReader(""), &stdout, &stderr)
...
func TestCLIHelpWithoutConfig(t *testing.T) {
	...
	for _, option := range []string{"--help", "-h", "--version"} {
		for _, args := range [][]string{{option}, {"--config", path, "--project", "/missing-project", option, "setup"}} {
			var stdout, stderr bytes.Buffer
			if code := Run("dev", args, strings.NewReader(""), &stdout, &stderr); code != 0 || stdout.Len() == 0 || stderr.Len() != 0 {
	...
	for _, option := range []string{"--help", "--version"} {
		var stdout, stderr bytes.Buffer
		if code := Run("dev", []string{option}, strings.NewReader(""), &stdout, &stderr); code != 0 || stderr.Len() != 0 {
...
func TestCLIProjectAndOverrides(t *testing.T) {
	...
	for _, tt := range []struct{ project, want string }{{".", "requires terminal"}, {"missing", "inspect project"}, {"file", "not a directory"}} {
		var stdout, stderr bytes.Buffer
		code := Run("dev", []string{"--config", "custom.json", "--project", tt.project}, strings.NewReader(""), &stdout, &stderr)
...
func TestCLIOutputFailure(t *testing.T) {
	var stderr bytes.Buffer
	if got := Run("dev", []string{"--version"}, strings.NewReader(""), failingWriter{}, &stderr); got != 1 {
