...
- The owner explicitly authorized tagging, pushing and publishing `v0.1.0` as a normal GitHub release and promoting it to stable/latest; publication completed on 2026-09-06. This supersedes the earlier `v0.x` prerelease / first-usable `v1.0.0` plan; no `v1.0.0` gate or separate release ceremony is required. Only tags containing a SemVer prerelease suffix are prereleases. [Public installation verification](docs/release-v0.1.0.md) records the live URLs and tested bytes. Future releases require separate authorization.
...
Public installer URLs for `v0.1.0` and latest are verified in [release evidence](docs/release-v0.1.0.md). A checksum fetched from the same release protects against corruption, not a compromised release publisher. Signing, notarization and attestations are out of scope. Homebrew, Windows, self-update, and package-manager submissions are deferred.
