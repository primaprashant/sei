...
- The owner explicitly authorizes tagging, pushing and publishing `v0.1.0` as a normal GitHub release and promoting it to stable/latest. This supersedes the earlier `v0.x` prerelease / first-usable `v1.0.0` plan; no `v1.0.0` gate or separate release ceremony is required. Only tags containing a SemVer prerelease suffix are prereleases. The parent release task performs the authorized remote actions; this local policy update does not. Public URLs remain pending until publication and live verification are recorded.
